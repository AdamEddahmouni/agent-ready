...
files the invoking user/CI job already has read access to. There is no
network service, no account, and no remote code path in this phase. The
threat model here is specifically about **untrusted repository content**
— a malicious or malformed `agent-ready.yaml`, or adversarial paths within
it — not about a multi-tenant or networked attacker model.

## Assets

- The integrity of the machine running Agent-Ready (no arbitrary code
  execution triggered by contract content).
- Files outside the repository boundary (no path-traversal read/write).
